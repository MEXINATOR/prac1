...
int main(void){
	...
	for (;;){
		...
		//Toggle Seconds LED
		//Write your logic here
		if(digitalRead(LED) == 0){ //LED PIN IS LOW
			digitalWrite(LED, HIGH);
		}
		else if(digitalRead(LED) == 1){ //LED PIN IS HIGH
			digitalWrite(LED, LOW);
		}
...
void hourInc(void){
	...
	if (interruptTime - lastInterruptTime>200){
		...
		//Fetch RTC Time
		int hrs = wiringPiI2CReadReg8(RTC, HOUR_REGISTER);
		//Increase hours by 1, ensuring not to overflow
		hrs ++;
		if(hrs > 23){hrs = 0;}
		//Write hours back to the RTC
		wiringPiI2CWriteReg8(RTC, HOUR_REGISTER, decCompensation(hrs));
	}
...
void minInc(void){
	...
	if (interruptTime - lastInterruptTime>200){
		...
		//Fetch RTC Time
		int min = wiringPiI2CReadReg8(RTC,MIN_REGISTER);
